#!/usr/bin/env python3
# jobs/ingest_sales_territory.py
#
# Bronze ingest for SalesTerritory.csv 
# Driven by configs/schemas/SalesTerritory.json 
...
from libs.utils import (
    ...
    load_spec_and_schema,   # -> (spec_dict, schema_structtype) and sets spec["schema"]
...
def parse_args():
    ap = make_bronze_ingest_parser(
        "Bronze ingest for SalesTerritory.csv (with preprocessing)",
        src_help="Path to source/SalesTerritory.csv",
        schema_help="Path to configs/schemas/SalesTerritory.json",
    )
...
def main():
    ...
    spark = (
        ...
        .appName("bronze_ingest__SalesTerritory")
    ...
    try:
        ...
        # 2) Preprocess raw file to UTF-8 temp (if configured)
        ...
        # 5) Write Bronze
        table_name   = spec.get("name") or spec.get("table") or "SalesTerritory"
        ...
        out_dir = write_bronze(
            ...
            ingestion_date=args.ingestion_date,   # None -> defaults to today in write_bronze
